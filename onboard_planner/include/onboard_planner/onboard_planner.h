...
class WayPointsTracker
{
  ...
  // of waypoints. Best velocity/setpoint is chosen according to the given duration.
  // When duration is not long enough, the planner will take position
  // arriving as first priority. If distance between two waypoints is less
  // than radius tolerance, zero velocity/setpoint will be given until duration is
  // ran out. If all waypoints are tracked, the last waypoint will be an attractor.
public:
  ...
  WayPointsTracker(bool VelocityControl, bool LoopMode, double MaxHorizVel,
                   ...
                   double TolR, double TolYaw, std::vector<WayPoint> AllWayPoints);
