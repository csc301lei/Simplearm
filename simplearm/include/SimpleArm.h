...
class SimpleArm
{
  public:
	...
	void callbackqrPose(const QR_position::Num::ConstPtr &msg);
	void calcJointAngle(ArmPos &armpos);
	void calcJointVel(ArmPos &armpos);
	void ServoPosControl(double theta_1, double theta_2, double theta1_vel, double theta2_vel);
	void ServoVelControl(double theta1_vel, double theta2_vel);
	double vel_limit(double vel);
